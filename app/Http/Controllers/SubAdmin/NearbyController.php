...
class NearbyController extends Controller {
    ...
    public function index() {
        ...
        return view('subadmin.nearby.index', ['js' => $js, 'css' => $css]);
    }
    ...
    public function create(Request $request) {
        try {
            if ($request->isMethod("post")) {
                ...
                if ($validator->fails()) {
                    return redirect()->route('subadmin.nearby.add')->withErrors($validator)->withInput();
                }


                $place = new ResortNearbyPlace();
                $place->name = $request->place_name;
                $place->resort_id = $request->get("subadminResort");
                ...
                if ($place->save()) {
                    ...
                    return redirect()->route('subadmin.nearby.index')->with('status', 'Nearby place has been added successfully.');
                } else {
                    return redirect()->route('subadmin.nearby.add')->with('error', 'Something went be wrong.');
                }
            }
            $css = [
            ...
            return view('subadmin.nearby.create-nearby', [
                'js' => $js,
                'css' => $css,
                'resorts' => $resorts,
                'states' => $states
            ...
            return redirect()->route('subadmin.nearby.index')->with('error', $ex->getMessage());
        }
    }
        ...
        try {
            ...
            if ($request->isMethod("post")) {
                ...
                if ($data->save()) {
                    ...
                    return redirect()->route('subadmin.nearby.edit', $id)->with('status', 'Nearby place has been updated successfully.');
                }
            }
            ...
            return view('subadmin.nearby.edit-nearby', [
                'css' => $css,
                'js' => $js,
                'data' => $data,
                'resorts' => $resorts,
            ...
            return redirect()->route('subadmin.nearby.index', $id)->with('error', $ex->getMessage());
        }
    }
