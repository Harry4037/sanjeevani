...
class RoomtypeController extends Controller {
    ...
    public function index(Request $request) {
        ...
        return view('subadmin.room.index', ['js' => $js, 'css' => $css]);
    }
    ...
    public function roomList(Request $request) {
        try {
            ...
            foreach ($rooms as $key => $room) {
                ...
                $roomsArray[$key]['action'] = '<a href="' . route('subadmin.room.edit', $room->id) . '" class="btn btn-info btn-xs"><i class="fa fa-pencil"></i> Edit </a>'
    ...
    public function create(Request $request) {
        try {
            if ($request->isMethod("post")) {
                ...
                if ($validator->fails()) {
                    return redirect()->route('subadmin.room.add')->withErrors($validator)->withInput();
                }
                ...
                if ($room->save()) {
                    ...
                    return redirect()->route('subadmin.room.index')->with('status', 'Room Type has been added successfully.');
                } else {
                    return redirect()->route('subadmin.room.add')->with('error', 'Something went be wrong.');
                }
            }
            $css = ["vendors/dropzone/dist/dropzone.css",];
            $js = ['vendors/dropzone/dist/dropzone.js',];
            return view('subadmin.room.create', [
                'js' => $js,
                'css' => $css
            ]);
        } catch (\Exception $ex) {
    ...
    public function editRoom(Request $request, $id) {
        try {
            ...
            return view('subadmin.room.edit', [
                'data' => $data,
                'css' => $css,
                'js' => $js,
