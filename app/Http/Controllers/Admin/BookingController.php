    ...
    public function userBookings(Request $request, $user_id) {
        try {
            ...
            foreach ($userBookingDetails as $i => $userBookingDetail) {
                ...
                $roomBooking = RoomBooking::selectRaw(DB::raw('id, DATE_FORMAT(check_in, "%d-%m-%Y %r") as check_in, DATE_FORMAT(check_out, "%d-%m-%Y %r") as check_out'))
                                ->where("booking_id", $userBookingDetail->id)->first();
                ...
                if ($checkInTime > $currentDataTime && $checkOutTime > $currentDataTime) {
                    $stat = "Ongoing";
                } elseif ($currentDataTime > $checkInTime && $currentDataTime < $checkOutTime) {
                    $stat = "Current";
                } else {
                    $stat = "Completed";
                }
    ...
    public function createBooking(Request $request) {
        try {
            if ($request->isMethod("post")) {
                ...
                $bookingIds = UserBookingDetail::where("user_id", $user->id)->pluck("id");
                $existingRecord = RoomBooking::where("check_in", "<=", date("Y-m-d H:i:s", strtotime($request->check_in)))
                        ->where("check_out", ">=", date("Y-m-d H:i:s", strtotime($request->check_out)))
                        ->whereIn("booking_id", $bookingIds)
                        ->first();
                if ($existingRecord) {
                    return redirect()->route('admin.booking.create')->withErrors("Booking already exist with these date's.")->withInput();
                }
